...
#
# Launch with a SINGLE 1Password unlock for the whole bundle:
#   op run --env-file=op.env -- bash scripts/run_anchor.sh
# op resolves the token once and injects it into this process tree, so every
# case inherits it — no further prompts for the duration of the run.
#
...
# Re-probe auth between cases (see _run_lib.sh): halt on a closed usage window
# rather than converting the rest of the bundle into error rows.
PRECASE_CHECK=probe_anchor_auth
